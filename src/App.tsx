/* 
   리액트는 컴포넌트(component단위 개발
   컴포넌트: 페이지, 세부화면, 위젯, 아이콘, 버튼
   컴포넌트: 재사용이 가능한 UI 조각
   컴포넌트: 템플릿(틀) + 작동코드

   리액트 컴포넌트는 파일명과 함수명을 (파스칼케이스로작성 )첫대문자 
   원래 리액트 컴포넌트 class/function 2가지 형태였고,
   기본이 class 형식이었음
...
const App = () => {
  ...
  const handleClickSecondaryPrimaryButton = () => {
    alert("Cancel");
  };
  ...
  return (
    <div>
      ...
      <Button
        label="Cancel"
        color="secondary"
        onClick={handleClickSecondaryPrimaryButton}
      />
    </div>
  );
};
